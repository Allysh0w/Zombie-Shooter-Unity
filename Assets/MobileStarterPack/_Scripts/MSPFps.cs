...
public class MSPFps : MonoBehaviour {
    ...
    //private GameObject ZumbiParticle = Resources.Load("Assets/CartoonZombies/effect/explosionSphere");
		...
		if (CurrentWeapon.firearms){
			if (Time.time > CurrentWeapon.nextFireTime + CurrentWeapon.fireRate){
				...
				if (hit.collider){
                    ...
                    {
                        Instantiate(Resources.Load("BloodSplatEffect"), hit.point, Quaternion.identity);
                        hit.collider.GetComponent<EnemyControllerZumbi>().ZumbiDamage();
                        //BloodSplatEffect
                    }
                    else
                    {
	...
	public void Hit(int power){
		...
		if(Health>1 && TBHA ==0){
            ...
            //TBHA = TimeBeforeHitAgain; // Tempo para receber o próximo Hit;
            TBHA = 1.0f;                       
